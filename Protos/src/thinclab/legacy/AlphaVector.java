...
public class AlphaVector implements Serializable {
	...
	public static AlphaVector dpBackup(IPOMDP ipomdp, DD belState, DD[] factoredBelState, DD[] primedV,
		...
		nextBelStates = NextBelState.oneStepNZPrimeBelStatesCached(ipomdp, belState, true, smallestProb);
		...
		for (String Ai : ipomdp.getActions()) {
			...
			actValue = actValue + ipomdp.discFact * nextBelStates.get(Ai).getSumObsValues();
			...
			if (MySet.find(bestObsStrat, alphaId) >= 0) {
				...
				for (int obsId = 0; obsId < nObservations; obsId++) {
					...
					if (bestObsStrat[obsId] == alphaId) {
						...
						obsConfig = IPOMDP.statedecode(obsId + 1, ipomdp.obsIVarIndices.length,
								ArrayUtils.subarray(ipomdp.obsVarsArity, 0, ipomdp.obsIVarIndices.length));

						obsDd = OP.add(obsDd,
								Config.convert2dd(IPOMDP.stackArray(ipomdp.obsIVarPrimeIndices, obsConfig)));
		...
		/* Add P(AJ | MJ) to Ti */
		DD[] Ti = ArrayUtils.add(ipomdp.currentTi.get(bestAction), ipomdp.currentAjGivenMj);

		Ti = ArrayUtils.add(Ti, ipomdp.currentThetajGivenMj);

		DD[] valFnArray = ArrayUtils.addAll(ArrayUtils.addAll(ipomdp.currentOi.get(bestAction), Ti),
				new DD[] { mjTransition, nextValFn });

		DD V = OP.addMultVarElim(valFnArray, new int[] { ipomdp.thetaVarPosition + 1, ipomdp.AjVarStartPosition + 1 });

		newAlpha = OP.addMultVarElim(V,
				ArrayUtils.addAll(ArrayUtils.subarray(ipomdp.stateVarPrimeIndices, 0, ipomdp.thetaVarPosition),
						ipomdp.obsIVarPrimeIndices));

//		newAlpha = 
//				OP.addMultVarElim(newAlpha, new int[] {ipomdp.thetaVarPosition + 1, ipomdp.AjVarStartPosition + 1});

		newAlpha = OP.addN(IPOMDP.concatenateArray(newAlpha, ipomdp.currentRi.get(bestAction)));
		...
		AlphaVector returnAlpha = new AlphaVector(newAlpha, bestValue, ipomdp.getActions().indexOf(bestAction),
				bestObsStrat);
	...
	public static AlphaVector dpBackup2(IPOMDP ipomdp, DD belState, DD[] factoredBelState, DD[] primedV,
		...
		int[] allVarIndices = ArrayUtils.addAll(
		...
		DD[] valFnArray = ArrayUtils.addAll(ipomdp.currentTi.get(bestAction), ipomdp.currentOi.get(bestAction));

		valFnArray = ArrayUtils.addAll(valFnArray, new DD[] { ipomdp.currentTauXPAjGivenMjXPThetajGivenMj, nextValFn });

		newAlpha = OP.addMultVarElim(valFnArray, sumoutIndices);
	...
	public static AlphaVector dpBackup2(IPOMDP ipomdp, DD belState, DD[] primedV, double maxAbsVal, int numAlpha)
		...
		/*
		 * A more efficient version of the backup using dot product for computing PBVs
		 * 
		 */
		...
		bestValue = OP.dotProduct(belState, newAlpha, stateVarIndices);
	...
	public static AlphaVector dpBackup(POMDP pomdp, DD[] belState, DD[] primedV, double maxAbsVal, int numAlphas) {
		...
		for (int actId = 0; actId < pomdp.nActions; actId++) {
			...
			actValue = actValue + pomdp.discFact * nextBelStates[actId].getSumObsValues();
		...
		for (int alphaId = 0; alphaId < numAlphas; alphaId++) {
			...
			if (MySet.find(bestObsStrat, alphaId) >= 0) {
				...
				for (int obsId = 0; obsId < pomdp.nObservations; obsId++) {
					...
					if (bestObsStrat[obsId] == alphaId) {
						obsConfig = POMDP.statedecode(obsId + 1, pomdp.nObsVars, pomdp.obsVarsArity);

						obsDd = OP.add(obsDd, Config.convert2dd(POMDP.stackArray(pomdp.primeObsIndices, obsConfig)));
		...
		newAlpha = OP.addMultVarElim(
				ArrayUtils.addAll(pomdp.actions[bestActId].transFn,
						ArrayUtils.addAll(pomdp.actions[bestActId].obsFn, nextValFn)),
				ArrayUtils.addAll(pomdp.primeVarIndices, pomdp.primeObsIndices));

		newAlpha = OP.addN(ArrayUtils.addAll(new DD[] { newAlpha }, pomdp.actions[bestActId].rewFn));
	...
	public static AlphaVector dpBackup(POMDP pomdp, DD belState, DD[] primedV, double maxAbsVal, int numAlphas) {
		...
		for (String action : pomdp.getActions()) {
			...
			/* compute immediate rewards */
			actValue = actValue
					+ OP.dotProduct(belState, pomdp.getRewardFunctionForAction(action), pomdp.getStateVarIndices());

			/* compute observation strategy */
			nextBelStates.get(action).getObsStrat();
			actValue = actValue + pomdp.discFact * nextBelStates.get(action).getSumObsValues();
