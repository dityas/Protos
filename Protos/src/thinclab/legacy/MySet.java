...
public class MySet {
	...
	public static String toString(int[] set) {
		...
		if (set != null) {
			...
			for (int i = 0; i < set.length; i++) {
				...
				if (i != set.length - 1)
					str = str + Integer.toString(set[i]) + new String(",");
				else
					str = str + Integer.toString(set[i]);
			}
		}
	...
	public static String maskToString(boolean[] mask) {
		...
		for (int i = 0; i < mask.length; i++) {
			...
			if (mask[i]) {
				...
				if (i != mask.length - 1)
					str = str + Integer.toString(i) + new String(",");
				else
	...
	public static int[] unionOrdered(int[] set1, int[] set2) {
		...
		if (i1 < set1.length)
			count += set1.length - i1;
		if (i2 < set2.length)
			count += set2.length - i2;
		...
		for (; i1 < set1.length; i1++) {

			set[count] = set1[i1];
			count++;
		}
		for (; i2 < set2.length; i2++) {
