...
public class NextBelState implements Serializable {
	...
	public void restrictN(DD[] marginals, int[][] obsConfig) {
		...
		for (int obsPtr = 0; obsPtr < numValidObs; obsPtr++) {
			...
			nextBelStates[obsPtr] = OP.restrictN(marginals, POMDP.stackArray(this.primeObsIndices, obsConfig[obsId]));
		}
	}

	/*
	 * get the observation values obsVals[i][j] is the value expected if we see
	 * observation i and then follow the conditional plan j
	 */
	...
	/*
	 * get observation strategy obsStrat[i] is the best conditional to plan to
	 * follow if observation i is seen this is just the index that maximizes over
	 * the obsVals and alphaValue is the value of that conditional plan given than
	 * observation
	 */
...
//				System.exit(-1);
	...
	public static HashMap<String, NextBelState> oneStepNZPrimeBelStatesCached(IPOMDP ipomdp, DD belState,
		...
		if (NextBelStateCache.cachingAllowed()) {
			HashMap<String, NextBelState> cachedEntry = NextBelStateCache.getCachedEntry(belState);
		...
		HashMap<String, NextBelState> nzPrimes = oneStepNZPrimeBelStates2(ipomdp, belState, normalize, smallestProb);
	...
	public static HashMap<String, NextBelState> oneStepNZPrimeBelStates2(IPOMDP ipomdp, DD belState, boolean normalize,
		...
		for (String act : ipomdp.getActions()) {
			...
			for (int o = 0; o < allObs.size(); o++) {
				...
				try {
					nextBelief = ipomdp.beliefUpdate(belState, act, allObs.get(o).stream().toArray(String[]::new));
				}
		...
		for (String Ai : ipomdp.getActions()) {
			...
			try {
				if (!nextBelStates.get(Ai).isempty()) {
					marginals = OP.marginals(((IBeliefOps) ipomdp.bOPs).getCpts2(belState, Ai),
							ArrayUtils.subarray(ipomdp.stateVarPrimeIndices, 0, ipomdp.thetaVarPosition),
							ArrayUtils.subarray(ipomdp.stateVarIndices, 0,
...
//					logger.debug("After computing marginals " + nextBelStates[actId]);
	...
	public static HashMap<String, NextBelState> oneStepNZPrimeBelStates(IPOMDP ipomdp, DD[] belState, boolean normalize,
		...
		int[] primeVarSubArray = ArrayUtils.subarray(ipomdp.stateVarPrimeIndices, 0, ipomdp.thetaVarPosition);
		...
		for (String Ai : ipomdp.getActions()) {
			...
			try {
				if (!nextBelStates.get(Ai).isempty()) {
					marginals = OP.marginals(bOps.getCpts2(belState, Ai), primeVarSubArray, ipomdp.stateVarIndices);
...
//					logger.debug("After computing marginals " + nextBelStates[actId]);
	...
	public static NextBelState[] oneStepNZPrimeBelStates(POMDP pomdp, DD[] belState, boolean normalize,
		...
		for (int actId = 0; actId < pomdp.nActions; actId++) {
			...
			dd_obsProbs = OP.addMultVarElim(
					ArrayUtils.addAll(belState,
							ArrayUtils.addAll(pomdp.actions[actId].transFn, pomdp.actions[actId].obsFn)),
			...
			if (!nextBelStates[actId].isempty()) {
				marginals = OP.marginals(
						ArrayUtils.addAll(belState,
								ArrayUtils.addAll(pomdp.actions[actId].transFn, pomdp.actions[actId].obsFn)),
						pomdp.primeVarIndices, pomdp.varIndices);
	...
	public static HashMap<String, NextBelState> oneStepNZPrimeBelStatesCached(POMDP pomdp, DD belState,
		...
		/* else compute and cache */
		HashMap<String, NextBelState> nzPrimes = oneStepNZPrimeBelStates(pomdp, belState, normalize, smallestProb);
		...
		for (String act : pomdp.getActions()) {
			...
			for (int o = 0; o < allObs.size(); o++) {
				...
				try {
					...
					nextBelief = pomdp.beliefUpdate(belState, act, allObs.get(o).stream().toArray(String[]::new));
				}
