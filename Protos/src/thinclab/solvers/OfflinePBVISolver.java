...
public class OfflinePBVISolver extends AlphaVectorPolicySolver {
	...
	private static final Logger LOGGER = LogManager.getLogger(OfflinePBVISolver.class);

	// -----------------------------------------------------------------------------------

	public OfflinePBVISolver(POMDP pomdp, BeliefRegionExpansionStrategy be, int numRounds, int numDpBackups) {
	...
	public void solve() {
		...
		if (this.expansionStrategy instanceof SSGABeliefExpansion) {
			...
			((SSGABeliefExpansion) this.expansionStrategy).setRecentPolicy(this.alphaVectors, this.policy);
		}
	...
	public String getActionForBelief(DD belief) {
		...
		return DecisionProcess.getActionFromPolicy((POMDP) f, belief, this.alphaVectors, this.policy);
	}
