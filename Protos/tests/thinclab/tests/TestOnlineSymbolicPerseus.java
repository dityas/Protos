...
class TestOnlineSymbolicPerseus {
	...
	void setUp() throws Exception {
		...
		this.l1multiple = "/home/adityas/git/repository/Protos/domains/tiger.L1multiple_new_parser.txt";
		this.l1Enemy = "/home/adityas/git/repository/Protos/domains/tiger.L1.enemy.txt";
//		this.l1multiple = 
//				"/home/adityas/UGA/THINCLab/DomainFiles/final_domains/cybersec.5S.2O.L1.2F.domain";

		CustomConfigurationFactory.initializeLogging();
		LOGGER = LogManager.getLogger(TestOnlineSymbolicPerseus.class);
	}
...
//		List<DD> T3 = fb.getBeliefPoints();
		...
		OnlineInteractiveSymbolicPerseus solver = new OnlineInteractiveSymbolicPerseus(tigerL1IPOMDP, fb, 1, 100);
...
//			DD obsDist = tigerL1IPOMDP.getObsDist(tigerL1IPOMDP.getCurrentBelief(), "NOP");
			...
			for (String action : tigerL1IPOMDP.getActions()) {
				...
				for (int s = 0; s < nextStateForAi.nextBelStates.length; s++) {
					LOGGER.debug("For Ai=" + action + " and o=" + tigerL1IPOMDP.getAllPossibleObservations().get(s)
							+ " belief is " + Arrays.toString(nextStateForAi.nextBelStates[s]));

					DD nextBelief = tigerL1IPOMDP.beliefUpdate(belief, action,
							tigerL1IPOMDP.obsCombinations.get(s).stream().toArray(String[]::new));

					LOGGER.debug("Factored belief is " + Arrays.toString(tigerL1IPOMDP.factorBelief(nextBelief)));

					for (int b = 0; b < nextStateForAi.nextBelStates[s].length - 1; b++) {

						DD marginal = nextStateForAi.nextBelStates[s][b];
						DD primedFactor = OP.primeVars(tigerL1IPOMDP.factorBelief(nextBelief)[b],
...
//			tigerL1IPOMDP.step(tigerL1IPOMDP.getCurrentBelief(), "NOP", obs);
	...
	void testIPBVISolverWithSSGAExpansion() throws Exception {
		...
		String l1DomainFile = "/home/adityas/UGA/THINCLab/DomainFiles/tiger.L1.F3.agnostic.domain";
		...
		OnlineIPBVISolver solver = new OnlineIPBVISolver(tigerL1IPOMDP, fb, 1, 100);
		...
		OnlineIPBVISolver ssgaSolver = new OnlineIPBVISolver(tigerL1IPOMDP, ssgaB, 5, 100);
	...
	void testISymbolicPerseusSolverWithSSGAExpansion() throws Exception {
		...
		OnlineInteractiveSymbolicPerseus ssgaSolver = new OnlineInteractiveSymbolicPerseus(tigerL1IPOMDP, ssgaB, 5,
				100);
...
//		CustomConfigurationFactory.setLogFileName("test.log");
		...
		solver.nextStep(solver.getActionAtCurrentBelief(), tigerL1IPOMDP.obsCombinations.get(2));
