...
public class MultiAgentSimulation extends Simulation {
	...
	private static final Logger LOGGER = LogManager.getLogger(MultiAgentSimulation.class);

	// ---------------------------------------------------------------------------------------

	public MultiAgentSimulation(BaseSolver ipomdpSolver, BaseSolver pomdpSolver, int interactions) {
...
//			if (!(this.solver instanceof HumanAgentSolver)) {
			...
			LOGGER.info("L0 belief is " + this.l0BeliefSequence.get(this.l0BeliefSequence.size() - 1));
			...
			this.stateSequence.add(this.solver.f.getBeliefString(currentState));
...
//			}
			...
			LOGGER.info("Agent j's expected reward is " + this.l0ExpectedReward.get(this.l0ExpectedReward.size() - 1));
			...
			if (this.summaryWriter != null) {
				this.summarizeInteraction(this.states.size(), stateJson, currentL1BeliefJson, currentL0BeliefJson,
						l1Action, l0Action, obs[0], obs[1], l1DiscountedReward, l0DiscountedReward);
			}
			...
			nextStateNode.setsBelief(this.solver.f.getBeliefString(this.states.get(this.states.size() - 1).toDD()));
			...
			this.putEdge(currentNode + 1, Arrays.asList(new String[] { l1Action, l0Action }), nextStateNode.getId());
...
//		if (!(this.solver instanceof HumanAgentSolver)) {
		...
		LOGGER.debug("Agent i took action " + actions[0] + ", agent j took action " + actions[1] + " in state "
	...
	public void logToFile(String fileName) {
		...
		try {
			...
			for (int i = 0; i < this.stateSequence.size(); i++) {
				...
				/* write belief of j */
				record.add("beliefJ", gsonHandler.fromJson(this.l0BeliefSequence.get(i), JsonObject.class));
				...
				/* write rewards of i */
				record.add("ERi", new JsonPrimitive(this.l1ExpectedReward.get(i)));

				/* write rewards of j */
				record.add("ERj", new JsonPrimitive(this.l0ExpectedReward.get(i)));
