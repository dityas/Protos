...
class TestBenchmarkNZPrimeStorage {
	...
	void testPBVComputation() throws Exception {
		...
		OnlineInteractiveSymbolicPerseus SP = new OnlineInteractiveSymbolicPerseus(ipomdp, BE, 1, 1);
		...
		for (int i = 0; i < 10; i++) {
			...
			for (String act : ipomdp.getActions()) {
				...
				for (int o = 0; o < allObs.size(); o++) {
					...
					DD nextBelief = ipomdp.beliefUpdate(ipomdp.getCurrentBelief(), act,
...
//		ipomdp.step(ipomdp.getCurrentBelief(), "listen", new String[] {"growl-left", "silence"});
		...
		for (int i = 0; i < 10; i++) {
			...
			HashMap<String, NextBelState> o = (HashMap<String, NextBelState>) oInStream.readObject();
