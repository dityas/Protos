...
public class POMDPSolver extends Executable {
	...
	public void solvePOMDP() {
		...
		this.solver = 
				new OfflineSymbolicPerseus(
						this.pomdp, 
						new SSGABeliefExpansion(this.pomdp, this.searchDepth, 30), 
						this.perseusRounds, 
						this.numDpBackups);
	...
	public void makePolicyGraph(String dirName) {
		...
		/* make policy graph */
		PolicyGraph G = new PolicyGraph((AlphaVectorPolicySolver) solver, 10);
		G.makeGraph();
		G.computeEU();
		
		G.writeDotFile(dirName, "policy_graph");
		
		PolicyTree T = new PolicyTree((OfflinePBVISolver) solver, 5);
		T.buildTree();
		T.computeEU();
		
		T.writeDotFile(dirName, "policy_tree");
		T.writeJSONFile(dirName, "policy_tree");
	}
