...
public class DDTree implements Serializable {
	...
	public void renameVar(String oldVar, String newVar) {
		 ...
		 */
		if (this.varName.contentEquals(oldVar))
			this.varName = newVar;
	...
	public DDTree atChild(String childName) throws Exception {
		...
		/* Child not found */
			...
			throw new Exception(this.varName + " does not contain child " + childName);
	}
	...
	public void setDDAt(List<String> childNames, DDTree ddToAppend) throws Exception {
		...
		if (childNamesList.size() == 0)
			this.setDDAt(child, ddToAppend);

		else
			this.children.get(child).setDDAt(childNamesList, ddToAppend);
	}
	...
	public DD toDD() {
		...
		/*
		 * Get the variable index. This is a little hacky. symbolic perseus appends "_P"
		 * to the end of primed variable names. In case of DDTree, primed variables are
		 * represented by "'" at the end. So here, we manually check if the variable is
		 * primed and if it is, look for varName + _P in the globals.
		 */
		if (this.varName.contains("'"))
			varIndex = Global.varNames.indexOf(this.varName.substring(0, this.varName.length() - 1) + "_P");

		else
			varIndex = Global.varNames.indexOf(this.varName);

		var values = Global.valNames.get(varIndex);
		...
		for (int c = 0; c < values.size(); c++) {
			children[c] = this.children.get(values.get(c)).toDD();
		}

		/*
		 * varIndex should always be incremented by 1 because the global arrays use
		 * Matlab-like indices.
		 */
