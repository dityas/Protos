...
public class OnlineInteractiveSymbolicPerseus extends OnlineIPBVISolver {
	...
	private static final Logger LOGGER = LogManager.getLogger(OnlineInteractiveSymbolicPerseus.class);

	// --------------------------------------------------------------------------------------

	public OnlineInteractiveSymbolicPerseus(IPOMDP ipomdp, BeliefRegionExpansionStrategy b, int maxRounds,
			int dpBackups) {
		/*
	...
	public void solveForBeliefs(List<DD> beliefs) {
		...
		try {
			...
			boundedPerseusStartFromCurrent(30, 0, this.dpBackups, beliefsArray, false);
	...
	public void boundedPerseusStartFromCurrent(int maxAlpha, int firstStep, int nSteps, DD[] unfactoredBeliefRegion,
		...
		int[] ipomdpVars = ArrayUtils.subarray(((IPOMDP) this.f).stateVarIndices, 0,
		...
		if (debug) {
			...
			for (int i = 0; i < numBeliefs; i++) {
				LOGGER.debug("Belief:" + i + " " + this.ipomdp.toMap(unfactoredBeliefRegion[i]));
			}
		...
		for (int stepId = firstStep; stepId < firstStep + nSteps; stepId++) {
			...
			for (int i = 0; i < this.alphaVectors.length; i++) {
				primedV[i] = OP.primeVars(this.alphaVectors[i], ipomdp.getNumVars());
			}
			...
			while (this.numNewAlphaVectors < maxAlphaSetSize && !permutedIds.isempty()) {

				if (nDpBackups >= 2 * this.alphaVectors.length) {
					this.computeMaxMinImprovement(numBeliefs);
					if (this.bestImprovement > this.ipomdp.tolerance && this.bestImprovement > -2 * this.worstDecline) {

						if (debug) {
							LOGGER.warn("Breaking because bestImprovement " + ipomdp.bestImprovement + ""
									+ "> tolerance " + ipomdp.tolerance + " && bestImprovement "
									+ ipomdp.bestImprovement + " > -2 * worstDecline " + (-2 * ipomdp.worstDecline));
						}
...
//				Global.newHashtables();
				...
				if (this.numNewAlphaVectors == 0)
					...
					maxcurrpbv = OP.getMax(this.currentPointBasedValues, permutedIds.permutation);
					maxnewpbv = OP.getMax(this.newPointBasedValues, this.numNewAlphaVectors, permutedIds.permutation);
				...
				if (this.numNewAlphaVectors < 1 || (OP.max(this.newPointBasedValues[i], this.numNewAlphaVectors)
						- OP.max(this.currentPointBasedValues[i]) < steptolerance)) {

					long beforeBackup = System.nanoTime();

					/* dpBackup */
					newVector = AlphaVector.dpBackup2(this.ipomdp, unfactoredBeliefRegion[i],
							/* beliefRegion[i], */
							primedV, maxAbsVal, this.alphaVectors.length);
...
//					newVector.alphaVector = OP.approximate(newVector.alphaVector, 0.001);
					...
					if (this.numNewAlphaVectors < 1)
						...
						improvement = OP
								.max(OP.sub(newValues, OP.getMax(this.newPointBasedValues, this.numNewAlphaVectors)));
			...
			for (int j = 0; j < unfactoredBeliefRegion.length; j++) {
				System.arraycopy(this.newPointBasedValues[j], 0, this.currentPointBasedValues[j], 0,
						this.numNewAlphaVectors);
			}
			...
			LOGGER.info("I: " + stepId + " \tB ERROR: " + String.format(Locale.US, "%.03f", bellmanErr)
					+ " \tUSED/TOTAL BELIEFS: " + numIter + "/" + numBeliefs + " \tA VECTORS: "
					+ this.alphaVectors.length + " \tAPPROX. CONV PATIENCE: " + this.numSimilar
					+ " \tNON DEC ERROR PATIENCE: " + this.errorPatience);

			if (Global.showProgressBar)
				Global.printProgressBar(stepId, this.dpBackups, this.maxRounds);
			...
			int numLeaves = Arrays.stream(this.alphaVectors).map(d -> d.getNumLeaves()).mapToInt(x -> x).sum();
			...
			if (this.declareApproxConvergenceForAlphaVectors(numLeaves, numIter, numBeliefs) && bellmanErr < 1.0) {
				LOGGER.warn("DECLARING APPROXIMATE CONVERGENCE AT ERROR: " + bellmanErr
	...
	public void logAlphaVectors() {
		...
		for (int i = 0; i < this.ipomdp.policy.length; i++) {
			LOGGER.debug("Alpha Vector " + i + " is " + this.alphaVectors[i] + " for action "
					+ this.ipomdp.getActions().get(this.ipomdp.policy[i]));
		}
	}
}
